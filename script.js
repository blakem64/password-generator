...
if(num<0.25){
  ///select special
   let el=special[Math.floor(Math.random()* special.length)]
  if(c1=="y"){
    output +=el;
    done++;
  }
  ...
  //select uppercase
  let el=uppercase[Math.floor(Math.random()* uppercase.length)]
   if (c3=="y"){
    output +=el;
    done++;
   }
  ...
  //select a number
  let el=number[Math.floor(Math.random() * number.length)]
    if(c4=="y"){
      output +=el;
      done++;
    }
